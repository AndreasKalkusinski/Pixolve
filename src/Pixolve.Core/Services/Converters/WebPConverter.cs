...
/// <summary>
/// Converts images to WebP format using SkiaSharp
/// </summary>
public class WebPConverter : IImageConverter
{
    public ImageFormat TargetFormat => ImageFormat.WebP;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var outputPath = GetOutputPath(imageFile, settings);
                ...
                using var data = await Task.Run(
                    () => image.Encode(SKEncodedImageFormat.Webp, settings.Quality),
                    cancellationToken);

                if (data == null)
                    return ConversionResult.Failure("Failed to encode image to WebP");
    ...
    private string GetOutputPath(ImageFile imageFile, ConversionSettings settings)
    {
